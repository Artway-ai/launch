...
def process_args(ctx):
    ...
    if 'TRAINER_ENDPOINTS' in ctx.envs:
        eps = ctx.envs['TRAINER_ENDPOINTS'].split(',')
        hosts = set([h.split(':')[0] for h in eps])
        ctx.args.master = eps[0] if ':' in eps[0] else '{}:6768'.format(eps[0])
        ctx.args.nnodes = len(hosts)
        ctx.logger.info('args reset by env TRAINER_ENDPOINTS\n{}'.format(
            eps))
...
enabled_plugins = [rewrite_host_ip, process_args, log]
